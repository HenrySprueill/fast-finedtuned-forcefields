...
def dataroot(folder: str):
    ...
    """
    assert isinstance(folder, str) and len(folder) > 0, \
...
def data_keys():
    ...
    Data keys used by default by the SchNet network
        ...
        * edge_attr Tensor[num_edges]: the pairwise distances for interacting
            atoms.
        * edge_index Tensor[2, num_edges]: the adjacency list defining the graph
             edges.
        * y Tensor[]: The energy target as a scalar.
        * num_nodes int: the number of atoms in the graph
    """
    return ("z", "edge_attr", "edge_index", "y")
